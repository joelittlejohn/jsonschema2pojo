...
public class DynamicPropertiesRule implements Rule<JDefinedClass, JDefinedClass> {
    ...
    /**
     * This rule adds dynamic getter, setter and builder methods based on the
     * properties and additional properties defined in a schema.
     * <p>
     * If accessors are being generated, then methods for getting and setting
     * properties by name will be added. These methods first attempt to call the
     * appropriate getter or setter for the property. If the named property is not
     * defined, then the additional properties map is used.
     * <p>
     * If builders are being generated, then a method for building properties by
     * name will be added. This method first attempts to call the builder for the
     * property. If no property with the supplied name is defined, then the
     * additional properties map is used.
     * <p>
     * The methods generated by this class throw an IllegalArgumentException, if the
     * name specified for the property is unknown and additional properties are not
     * enabled. A ClassCastException will be thrown, when the value being set is
     * incompatible with the type of the named property.
     *
     * @param nodeName
     *            the name of the node for which dynamic getters, setters, and
     *            builders are being added.
     * @param node
    ...
    JFieldRef getOrAddNotFoundVar(JDefinedClass jclass) {
        jclass.field(PROTECTED | STATIC | FINAL, Object.class, NOT_FOUND_VALUE_FIELD, _new(jclass.owner()._ref(Object.class)));
    ...
    private JInvocation illegalArgumentInvocation(JDefinedClass jclass, JVar propertyName) {
        return _new(jclass.owner()._ref(IllegalArgumentException.class)).arg(lit("property \"").plus(propertyName).plus(lit("\" is not defined")));
    }

    private JInvocation illegalArgumentInvocation(JDefinedClass jclass, String propertyName, JType propertyType, JVar valueVar) {
        return _new(jclass.owner()._ref(IllegalArgumentException.class)).arg(lit("property \"" + propertyName + "\" is of type \"" + propertyType.fullName() + "\", but got ").plus(valueVar.invoke("getClass").invoke("toString")));
    }
