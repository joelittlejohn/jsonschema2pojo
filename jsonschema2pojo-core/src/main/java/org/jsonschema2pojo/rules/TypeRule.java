...
public class TypeRule implements Rule<JClassContainer, JType> {
     ...
     * <p>
     * When applied, this rule reads the details of the given node to determine the
     * appropriate Java type to return. This may be a newly generated type, it may
     * be a primitive type or other type such as {@link java.lang.String} or
     ...
     * @return the Java type which, after reading the details of the given schema
     *         node, most appropriately matches the "type" specified
     */
    @Override
    ...
    private String getTypeName(JsonNode node) {
        if (node.has("type")) {
            ...
            if (typeNode.isArray() && typeNode.size() > 0) {
                final List<String> typeValues = StreamSupport.stream(typeNode.spliterator(), false).map(JsonNode::asText).filter(n -> !"null".equals(n)).collect(Collectors.toList());
